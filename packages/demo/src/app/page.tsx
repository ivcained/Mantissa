...
export default function Home() {
  ...
  const getEthereumProvider = () => {
    ...
    if (eth.isPhantom && !eth.isMetaMask) {
      console.warn("Phantom detected as default. Please disable Phantom or use a profile with only MetaMask.");
    }
  ...
  const signMessage = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < 32; i++) challenge[i] = msgBytes[i % msgBytes.length] || 0;
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="bg-[rgba(2,13,8,0.4)] rounded-[20px] p-6 md:p-8 backdrop-blur-sm">
          ...
          {!isSupported ? (
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[var(--text-secondary)] font-mono text-xs mb-1 block pl-1">MESSAGE TO SIGN</label>
                  <input
                    type="text"
          ...
          {error && (
            <div className="mt-6 bg-[rgba(255,50,50,0.1)] border border-red-500/50 rounded-xl p-4 animate-in fade-in slide-in-from-top-2">
              <p className="text-red-400 text-sm font-mono text-center break-words">{error}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-[rgba(0,0,0,0.5)] rounded-lg p-3 font-mono text-sm h-48 overflow-y-auto space-y-1 custom-scrollbar">
            {logs.map((log, i) => (
              <div key={i} className="flex gap-2">
                ...
                <span className="text-[var(--foreground)] break-all">{log.split(']')[1]}</span>
              </div>
            ))}
      ...
      <div className="mt-12 text-center">
        <div className="inline-flex items-center gap-6 text-[var(--text-secondary)] text-xs font-mono tracking-widest opacity-60">
          ...
          <span>•</span>
          <span>RPC: {RPC_URL.includes('localhost') ? 'LOCALHOST' : 'TESTNET'}</span>
        </div>
      </div>
    </div>
  );
